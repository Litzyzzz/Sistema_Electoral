        ...
        <div class="panel-main">
            <aside class="panel-sidebar">
                ...
                <nav class="panel-nav">
                    ...
                    <a class="panel-nav-link active" href="{{ route('resultados.bandera') }}">
            ...
            <div class="panel-content">
                <header class="panel-topbar">
                    <h1 class="panel-title">Resultados por bandera</h1>
                        ...
                        @foreach($ranking as $index => $candidato)
                            <article class="result-card">
                                <div class="result-image-wrap">
                                    <img
                                        class="result-image"
                                        src="{{ asset('img/partidos/' . $candidato->bandera) }}"
                                        alt="Bandera de {{ $candidato->nombre_partido }}"
                                    >
                                    <span class="result-rank rank-{{ min($index + 1, 3) }}">{{ $index + 1 }}</span>
                                </div>

                                <div class="result-info">
                                    <h2 class="result-name">{{ $candidato->nombre_partido }}</h2>
                                    <p class="result-party">{{ $candidato->nombre_candidato }}</p>
